# Install SUMO and OMNET++ Latest version


#Generating results

--> Run: GEMV2PackageV1.2/runSimulation.m
...
How to create:
1. Create the .sumocfg for the required area of simulation using the SUMO-OSM-Web-Wizard.
2. Convert the .sumocfg file to the mobility .xml file using the following command:

sumo.exe -c <InputFilename>.sumocfg --fcd-output <OutputMobilityFilename>.xml --fcd-output.geo true --netstate-dump <OutputMobilityFilename>.net.xml 
...
5. Run the VeCEN configuration file. 
...
#Extra notes:
--> All the steps for producing the above simulation files are practically shown in the video uploaded.
